...
function tambah_data($data)
{
    ...
    if (mysqli_num_rows($sqlselect) === 0) {
        ...
        $query = "UPDATE tb_total_nilai SET id_tahunajaran = 15, id_kelas = 6 WHERE nisn='$nisn'";
        $sql = mysqli_query($GLOBALS['conn'], $query);
...
function ubah_data($data)
{
    ...
    $queryShow = "SELECT * FROM tb_nilai WHERE nisn = '$nisn' AND id_mapel = 1 AND id_tahunajaran = 15 AND id_kelas = 6;";
    $sqlShow = mysqli_query($GLOBALS['conn'], $queryShow);
    $result = mysqli_fetch_assoc($sqlShow);

    $query = "UPDATE tb_nilai SET sumatif='$sumatif', sumatif_akhir='$sumatif_akhir', nilai_rapor='$nilai_rapor' WHERE nisn='$nisn' AND id_mapel = 1 AND id_tahunajaran = 15 AND id_kelas = 6;";
    $sql = mysqli_query($GLOBALS['conn'], $query);
...
function hapus_data($data)
{
    ...
    $query = "DELETE FROM tb_nilai WHERE nisn = '$nisn' AND id_mapel = 1 AND id_tahunajaran = 15 AND id_kelas = 6;";
    $sql = mysqli_query($GLOBALS['conn'], $query);
    ...
    $query = "UPDATE tb_total_nilai SET ipas = 0 WHERE nisn='$nisn' AND id_tahunajaran = 15 AND id_kelas = 6;";
    $sql = mysqli_query($GLOBALS['conn'], $query);
