...
function tambah_data($data)
{
    ...
    $query = "UPDATE tb_total_nilai SET seni_teater = '$nilai_rapor' WHERE nisn='$nisn' AND id_tahunajaran = 14 AND id_kelas = 6;";
    $sql = mysqli_query($GLOBALS['conn'], $query);
...
function ubah_data($data)
{
    ...
    $queryShow = "SELECT * FROM tb_nilai WHERE nisn = '$nisn' AND id_mapel = 7 AND id_tahunajaran = 14 AND id_kelas = 6;";
    $sqlShow = mysqli_query($GLOBALS['conn'], $queryShow);
    $result = mysqli_fetch_assoc($sqlShow);

    $query = "UPDATE tb_nilai SET sumatif='$sumatif', sumatif_akhir='$sumatif_akhir', nilai_rapor='$nilai_rapor' WHERE nisn='$nisn' AND id_mapel = 7 AND id_tahunajaran = 14 AND id_kelas = 6;";
    $sql = mysqli_query($GLOBALS['conn'], $query);
